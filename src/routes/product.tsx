...
export const Route = createFileRoute("/product")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Robots running use cases autonomously. Real home environments. Zero human operators. 24/7." },
      { property: "og:title", content: "The Foundry — Lili-o" },
      { property: "og:description", content: "How Lili-o generates the data Physical AI needs." },
    ],
  }),
  component: Product,
});
...
const tableRows: { label: string; values: (string | { v: string; hi?: boolean })[] }[] = [
  ...
  { label: "Companies", values: ["Lightwheel · NVIDIA", "Scale · Senseirobotic", "Tutor · Figure · Agibot", { v: "Lili-o", hi: true }] },
];
  ...
  return (
    <div className="theme-dark bg-ink text-paper min-h-screen">
      ...
      <section className="container-x py-28 md:py-36 grid md:grid-cols-2 gap-16 items-center">
        <div>
          ...
          <div className="mt-8 space-y-5 text-paper/70 leading-relaxed">
            <p>Today, most robotics data is collected through teleoperation — a human operates a robot, one task at a time. It's slow, expensive, and impossible to scale. And none of it happens in kitchens.</p>
            <p>The Lili-o foundry runs household tasks autonomously across 50+ purpose-built home environments — kitchens, living rooms, bathrooms, laundry rooms — robots operate 24/7, retry on failure, and generate episodes continuously. No human operator. No wage floor. No ceiling on scale.</p>
          </div>
          <div className="mt-8 flex flex-wrap gap-2">
            {["Kitchen", "Living room", "Bathroom", "Laundry room", "Bedroom"].map((room) => (
              <span key={room} className="rounded-full border border-white/10 px-3 py-1 text-xs text-paper/50">{room}</span>
            ))}
          </div>
          <div className="mt-8 grid grid-cols-3 gap-4">
            {[{ k: "24/7", l: "operation" }, { k: "50+", l: "home environments" }, { k: "0", l: "operators" }].map((s) => (
        ...
        <div className="relative">
          <img src={foundryImg} alt="Lili-o foundry environments" loading="lazy" width={1400} height={1000} className="w-full h-full object-cover rounded-2xl border border-white/10" />
        </div>
      </section>
      ...
      <section className="container-x py-28 md:py-36">
        ...
        <h2 className="display-lg max-w-4xl">One use case. Deployed everywhere. Running forever.</h2>
        ...
        <div className="mt-16 grid md:grid-cols-3 gap-px bg-white/10">
          {[
            { n: "01", t: "Hardware agnostic", d: "One use case, any robot. Unitree, Rainbow, Agibot — no reprogramming." },
            { n: "02", t: "One-Shot learning", d: "5-minute demonstration → autonomous skill. Industry standard: 1 hour+." },
            { n: "03", t: "Perpetual output", d: "One build. Infinite episodes. 24/7 generation across the entire fleet." },
          ].map((s) => (
      ...
      <section className="bg-[#141414] border-y border-white/5">
        <div className="container-x py-28 md:py-36">
          ...
          <div className="mt-14 grid grid-cols-2 md:grid-cols-5 gap-3">
            {modalities.map((m, i) => (
              <div key={m.t} className="rounded-xl border border-white/10 p-6 hover:border-[var(--violet)] transition group">
          ...
          <p className="mt-10 text-paper/60 max-w-2xl">
            <span className="text-paper">5 modalities. Synchronized. Auto-labeled.</span> Cross-embodiment compatible. EU AI Act compliant.
          </p>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="grid md:grid-cols-[1fr_2fr] gap-16 mb-16">
          ...
          <div>
            ...
            <p className="mt-4 text-paper/60">This captures the environmental chaos, behavioral variance, and physical interaction that a controlled environment can never replicate. The mess on the counter. The wet dish. The awkward cabinet angle.</p>
            <div className="mt-8 flex flex-wrap gap-3">
              {["RGB-D", "Tactile / haptic", "Diverse home layouts", "Natural behavior", "EU AI Act ✓"].map((t) => (
                <span key={t} className="rounded-full border border-[var(--violet)]/40 text-[var(--violet)] px-4 py-2 text-sm">
                  {t}
                </span>
              ))}
            </div>
        ...
        {/* Household task examples */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-white/10">
          {[
            { room: "Kitchen", tasks: ["Dish washing", "Meal prep", "Appliance use", "Counter cleaning"] },
            { room: "Living room", tasks: ["Object sorting", "Table setting", "Tidying", "Vacuuming"] },
            { room: "Laundry room", tasks: ["Folding clothes", "Loading washer", "Ironing", "Sorting laundry"] },
            { room: "Bathroom", tasks: ["Surface wiping", "Bin handling", "Towel folding", "Cleaning fixtures"] },
          ].map((c) => (
      ...
      <section className="relative overflow-hidden border-t border-white/5">
        ...
        <div className="container-x py-32 md:py-40 relative text-center">
          <h2 className="display-xl max-w-4xl mx-auto">
            Ready to train on the data <span className="text-[var(--violet)]">Physical AI</span> has been missing?
          </h2>
